...
// 获取当天交易记录工具
server.tool("getTodayTransactions", "获取当天的财务交易记录 - 只返回今日的收支明细，相比获取全部记录更高效，减少上下文占用", {}, async () => {
...
// 获取交易统计工具
server.tool("getTransactionSummary", "获取财务统计汇总 - 计算总收入、总支出、余额等统计信息", {}, async () => {
          ...
          text: JSON.stringify(
            {
              ...
              supportedOperations: ["记录新的收入交易", "记录新的支出交易", "批量记录多笔交易", "查看所有交易记录", "查看当天交易记录", "获取交易统计汇总", "检查重复交易记录"],
